<?php
/**
 * GitPHP Controller User
 *
 * Controller for user web api
 *
 ...
 * @subpackage Controller
 */
//FIXME modify this code, make it not pointed to tree page...
class GitPHP_Controller_User extends GitPHP_ControllerBase
{
	...
	{
		$this->tpl->assign('extracss_file','page/tree');
	...
	{
		require_once(GITPHP_APPDIR . 'UserExport.class.php');
		$user_export = new GitPHP_UserExport(); 
		...
		if (isset($this->params['file'])) {
			...
			$view_data = 'dir';
			$parent_path = $tree->GetPath();
			if(strpos($parent_path, '/')){
				$parent_path = substr($parent_path, 0, strripos($parent_path, '/'));
				$parent_path = '&amp;h='.$commit->PathToHash($parent_path).'&amp;f='.$parent_path;
				if(isset($_GET['bn']))
					$parent_path = $parent_path . '&amp;bn=' . $_GET['bn'];
			...
			$this->tpl->assign('parent_path', $parent_path);
		}
		$this->tpl->assign('view_data', $view_data);
