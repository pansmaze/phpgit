...
class GitPHP_FileDiff
{
	...
	public function GetDiffSplit()
	{
		...
		if (function_exists('xdiff_string_diff')) {
			...
			$diffLines = explode("\n", $exe->Execute(GIT_DIFF, array("-U0", $this->fromHash, $this->toHash)));
		}
		...
		foreach($diffs as $d) {
			while($idx+1 < $d['line']) {
				//$h = iconv(GitPHP_Config::GetInstance()->GetValue('encoding'), "UTF-8", $blob[$idx]);
				//$h = mb_convert_encoding($blob[$idx], "UTF-8", GitPHP_Config::GetInstance()->GetValue('encoding'));
				$h = $blob[$idx];
				$output[] = array('', $h, $h);
				$idx ++;
			}
			...
			for($i = 0; $i < count($d['left']) || $i < count($d['right']); $i++) {
				...
				//$output[] = array($mode, iconv($this->project->GetCharset(), "UTF-8", $left), iconv($this->project->GetCharset(), "UTF-8", $right));
				$output[] = array($mode, mb_convert_encoding($left, "UTF-8", $this->project->GetCharset()), mb_convert_encoding($right, "UTF-8", $this->project->GetCharset()));
			}
