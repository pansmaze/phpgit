...
class GitPHP_Tree extends GitPHP_FilesystemObject
{
	 ...
	 * SetContentsCommit
	 *
	 * Sets the commit for every content of this tree
	 *
	 * @access public
	 */
	public function SetContentsCommit()
	{
		...
		foreach ($this->contents as $i=>$content)
		{
			...
			$content_history = array_shift($content_history);
	 ...
	 */
	public function GetContents($set_commit = false)
	{
	 ...
	 * SortTree
	 *
	 * Sort the tree content by file type
	 *
	 * @access public
	 * @return string cache key
	 */
	private function SortTree()
	{
		...
		{
			if($item instanceof GitPHP_Tree){
				$item->SetType('tree');
				$temp_tree[] = $item;
			}elseif($item instanceof GitPHP_Blob){
				if(preg_match("/^(.*)\.(jpg|png|gif|bmp|bnp)$/i",$item->GetName())){
					$item->SetType('image');
				}else{
					$item->SetType('blob');
				}
		...
		$this->contents = array_merge($temp_tree, $temp_blob);
	}
