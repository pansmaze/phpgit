...
/* log methods {{{2*/
	 ...
	 */
	public function GetLog($hash, $count = 50, $skip = 0, $fpath = false)
	{
		if (!is_array($fpath))
		{
			...
			//Get history of a fpath...
			$file_hash = $this->GetCommit($hash)->PathToHash($fpath[0]);
			if ('blob' == $fpath[1])
			{
				$file =  $this->GetBlob($file_hash);
			} elseif ('tree' == $fpath[1]) {
				$file =  $this->GetTree($file_hash);
			} else {
				throw new Exception('Parameter ftype is needed!');
			}
			$file->SetCommit($this->GetCommit($hash));
			$file->SetPath($fpath[0]);
...
//		if($fpath){
//			$hash = $this->GetCommit($hash)->PathToHash($fpath);
//		}
...
//			foreach($log as $i=>$val){
				//$this_path = $val->SearchFiles($fpath);
//				$this_hash = $this->GetCommit($hash)->PathToHash($fpath);
//				$this_path = $this->GetTree($this_hash)->GetPathTree();
//				$this_path = $this_path->GetPath();
//				var_dump($this_path);
//				var_dump($val instanceof GitPHP_Commit);
			//	var_dump ($this_path);
//				if($fpath == )
//			}
//		}
		if($count > 1){
			...
			for($i = 0; $i < $count; $i++ ){
				for($j = $count - 1; $j > $i; $j--){
					$t1 = intval($log[$j]->GetAuthorEpoch());
					$t2 = intval($log[$j-1]->GetAuthorEpoch());
					if($t1 > $t2){
						$tmp = $log[$j];
						$log[$j] = $log[$j-1];
						$log[$j-1] = $tmp;
