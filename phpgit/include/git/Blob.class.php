...
class GitPHP_Blob extends GitPHP_FilesystemObject
{
	...
	public function GetData($explode = false)
	{
		...
		if ($explode)
			return explode("\n",mb_convert_encoding($this->data, "UTF-8", $this->project->GetCharset()));
		else
			return mb_convert_encoding($this->data, "UTF-8", $this->project->GetCharset());
	}
	...
	private function ReadData()
	{
		...
		if ($this->GetProject()->GetCompat()) {
			...
			$this->data = $exe->Execute(GIT_CAT_FILE, $args, "GBK");
		} else {
