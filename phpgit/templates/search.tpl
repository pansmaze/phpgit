...
<table cellspacing="0">
  ...
  {foreach from=$results item=result}
    <tr class="{cycle values="light,dark"}">
      ...
      <td><a href="{$SCRIPT_NAME}?p={$project->GetProject()|urlencode}&amp;a=commit&amp;h={$result->GetHash()}" class="list commitTip" {if strlen($result->GetTitle()) > 50}title="{$result->GetTitle()|escape:'html'}"{/if}><strong>{$result->GetTitle(50)|escape:'html'}</strong>
