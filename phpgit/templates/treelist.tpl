...
{foreach from=$tree->GetContents(true) item=treeitem}
  ...
  <tr class="{cycle values="light,dark"}">
    ...
    <td title="{if $this_commit->GetAge() > 60*60*24*7*2}{$this_commit->GetAge()|agestring}{else}{$this_commit->GetCommitterEpoch()|date_format:"%Y-%m-%d"}{/if}">
    ...
    <td>
     {*<a href="{$SCRIPT_NAME}?p={$project->GetProject()|urlencode}&amp;a=snapshot&amp;h={$treeitem->GetHash()}&amp;f={$treeitem->GetPath(true)}&amp;bn={$branch_name}" class="snapshotTip">{t}snapshot{/t}</a>*}
      ...
      <a class="history_link" href="{$SCRIPT_NAME}?p={$project->GetProject()|urlencode}&amp;a=history&amp;h={$commit->GetHash()}&amp;f={$treeitem->GetPath(true)}&amp;bn={$branch_name}">{$this_commit->GetTitle(50)|escape:'html'}</a>
    </td>
  </tr>
{/foreach}
</tbody>
